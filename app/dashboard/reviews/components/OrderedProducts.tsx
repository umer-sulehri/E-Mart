...
interface OrderedProductsProps {
  /** Bumped by the parent when a review is submitted so sibling lists refresh. */
  onCreated?: () => void;
}

export default function OrderedProducts({ onCreated }: OrderedProductsProps) {
  ...
  const handleSubmit = async (item: ReviewableItem) => {
    ...
    try {
      ...
      toast.success(`Review submitted for ${item.name} and is pending approval`);
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        <div className="h-16 animate-pulse rounded-xl bg-muted-100" />
        <div className="h-64 animate-pulse rounded-xl bg-muted-100" />
      </div>
    );
  }
