  ...
  return (
    <nav
      ...
      style={{
        ...
        borderBottom: '1px solid rgba(217,176,140,0.15)',
      }}
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          {notifOpen && (
            <>
              ...
              <div className="absolute right-0 mt-2 w-80 rounded-2xl overflow-hidden z-50" style={{ background: 'var(--color-primary-dark)', border: '1px solid rgba(217,176,140,0.2)', boxShadow: '0 25px 50px rgba(0,0,0,0.1)' }}>
                ...
                {notifications.map(n => (
                  <div key={n.id} className="p-3 flex gap-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(217,176,140,0.15)' }}>
        ...
        <div className="relative">
          <button onClick={() => { setUserOpen(!userOpen); setNotifOpen(false); }} className="flex items-center gap-2 px-3 py-2 rounded-xl transition-colors hover:bg-white/5" style={{ color: 'rgba(255,255,255,0.8)' }}>
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ background: 'linear-gradient(135deg, #6B4E35, #3B2A1A)' }}>
          ...
          {userOpen && (
            <>
              ...
              <div className="absolute right-0 mt-2 w-56 rounded-2xl overflow-hidden z-50" style={{ background: 'var(--color-primary-dark)', border: '1px solid rgba(217,176,140,0.2)', boxShadow: '0 25px 50px rgba(0,0,0,0.1)' }}>
      ...
      {confirmLogout && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }} onClick={() => setConfirmLogout(false)}>
          <div className="w-full max-w-sm rounded-[16px] p-6 text-center" style={{ background: 'var(--color-primary-dark)', border: '1px solid rgba(217,176,140,0.2)' }} onClick={e => e.stopPropagation()}>
